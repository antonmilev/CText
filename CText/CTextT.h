...
#define ContS template<typename C, typename Val = typename C::value_type, typename X = std::enable_if_t<std::is_convertible<Val, std::basic_string<T> >::value || std::is_convertible<Val, const T*>::value >  >
#define ContC template<typename C, typename Val = typename C::value_type, typename X = std::enable_if_t<std::is_same<Val, T>::value || std::is_convertible<Val, std::basic_string<T> >::value || std::is_convertible<Val, const T* >::value  >  >
...
#define ContX template <typename K = std::size_t, typename C, typename V = std::remove_const_t<std::remove_reference_t<decltype(std::declval<C>()[std::declval<K>()])>>, typename X = std::enable_if_t<std::is_convertible<T*,V>::value>>
#define ContY template <typename K = std::size_t, typename C, typename V = std::remove_const_t<std::remove_reference_t<decltype(std::declval<C>()[std::declval<K>()])>>, typename X = std::enable_if_t<std::is_convertible<T, V>::value || std::is_convertible<T*, V>::value>>
...
public:
    ...
    ContC size_t collect(C& container, const T* cList) const;  // fill the container with substrings containing only characters from the list
    ContC size_t collectSubstrings(C& container, const C* start = nullptr, const C* end = nullptr, const C* contain = nullptr, const T* sep = Separators) const;  // fill the container with the found substrings after applying the conditions 
    ContC size_t collectPositions(RangeVector& pos, const C* start, const C* end, const C* has, const T* sep) const;
    ContC size_t collectLines(C& container, bool appendSeparators = false, const T* sep = SeparatorsLine) const;  // fills a STL container with the collected lines
    ContC size_t collectSentences(C& container, const T* sep = SeparatorsSentence, const T* sepWords = Separators) const;
    ContC size_t collectWords(C& container, const T* sep = Separators) const;  // fill container with all found words
    ...
    ContY bool   containAny(const C& container, size_t n, bool bCase = true) const;  
    ...
    ContX size_t findPositions(const C& list, size_t n, RangeVector& pos, bool bCase = true, std::vector<size_t>* indexes = nullptr) const;
    ...
    size_t       indexOfAny(const T* cList, size_t n, size_t from, bool bCase, size_t* idxList) const;  // find first instance of any character in the passed string, return substring index or -1
    ContS size_t indexOfAny(const C& container, size_t from = 0, bool bCase = true, size_t* idxList = nullptr) const;
    ContY size_t indexOfAny(const C& container, size_t n, size_t from = 0, bool bCase = true, size_t* idxList = nullptr) const; // find the first instance of the string in the list, return string index or -1 if not found
   ...
   ContX CTextT& removeAny(const C& container, size_t n, bool bCase = true); //remove any from the provided in the list strings
   ...
   ContX CTextT& replaceAny(const C& what, size_t n, const T c, bool bCase = true);
   ...
   ContX CTextT& replaceAny(const C& what, size_t n, const C& with, bool bCase = true);
   ContS CTextT& replaceAny(const C& container, const T* with, bool bCase = true);
   ContX CTextT& replaceAny(const C& container, size_t n, const T* with, bool bCase = true);
    CTextT&      replaceAt(const RangeVector& pos, const T* with);
    CTextT       replaceAt(const RangeVector& pos, const T* with) const;
   ContX CTextT& replaceAt(const RangeVector& pos, const std::vector<size_t>& indexes, const C& with) ; 
   ContX CTextT  replaceAt(const RangeVector& pos, const std::vector<size_t>& indexes, const C& with) const;
   ...
   ContX CTextT& reverseAny(const C& container, size_t n, bool bCase = true);
    ...
    ContC size_t split(C& container, bool appendSeparators = false, const T* sep = Separators) const;  // fills the STL container with the collected substrings
    ...
    bool         startsWithAny(const T* cList, size_t from = 0, bool bCase = true, size_t* idx = nullptr) const;   //return true if starts with any of the provided characters, the optional parameter chFound will contain the found character
    ContC bool   startsWithAny(const C& container, size_t from = 0, bool bCase = true, size_t* idx = nullptr) const;
    ContY bool   startsWithAny(const C& container, size_t n, size_t from = 0, bool bCase = true, size_t* idx = nullptr) const;
...
protected:
    ContC static void Permute(C& container, CTextT& a, size_t l, size_t r);
