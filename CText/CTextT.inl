...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
void CTextT<T>::equalRange(T cBegin, T cEnd)
{
    clear();
    append(cBegin, cEnd);
}
...
{
    size_t n = Strlen(cList);
    return (indexOfAny(cList, n, 0, bCase) != std::string::npos);
}
...
{
    return containAny(container, container.size(), bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
bool CTextT<T>::containAny(const C& container, size_t n, bool bCase) const
{
    return (indexOfAny(container, n, 0, bCase) != std::string::npos);
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
T CTextT<T>::operator[](int pos) const
{
    return m_str[pos];
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::containOnly(T c, bool bCase) const
{
    return StrFindChNot(str(), c, bCase) == nullptr;
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
const T* CTextT<T>::findAny(const T* cList, bool bCase) const
{
    return StrFindChAny(str(), cList, bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
const T* CTextT<T>::findNot(const T* cList, bool bCase) const
{
    return StrFindChNot(str(), cList, bCase);
}


//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::contain(const T* s) const
{
    if(s == nullptr)
        return false;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::appendUnique(T ch)
{
    if(contain(ch))
        return false;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::appendUnique(const T* str)
{
    if(contain(str))
        return false;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::startsWith(const T& ch, size_t from, bool bCase) const
{
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::startsWith(const T* s, size_t from, bool bCase, size_t* offset) const
{
    if(isEmpty() || EmptyOrNull(s)|| from >= length())
        return false;

    return StartsWith(str(from),s,bCase,-1,offset);
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::startsWithAny(const T* cList, size_t from, bool bCase, size_t* idx) const
{
    if(from >= length() || EmptyOrNull(cList))
        return false;
    bool bRes = IsOneOf(m_str[from], cList, bCase);
    if(bRes)
        if(idx) *idx = from;
    return bRes;
}
...
{
    return startsWithAny(list, list.size(), from, bCase, idx);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
bool CTextT<T>::startsWithAny(const C& list, size_t n, size_t from, bool bCase, size_t* idx) const
{
    size_t pos = 0;
    for(size_t i = 0; i < n; i++)
    {
        if(startsWith(list[i], from, bCase))
        {
            if(idx) *idx = pos;
            return true;
        }
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::endsWith(const T* s, size_t from, bool bCase) const
{
    if(isEmpty() || EmptyOrNull(s) || from >= length())
        return false;
    ...
    return EndsWith(str(), s, bCase, from);
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
const T* CTextT<T>::StrFindChNot(const T* s, T c, bool bCase)
{
    if(!bCase) c = upper(c);
...
{
    return indexOfAny(cList, std::string::npos, from, bCase, nullptr);
}
//-----------------------------------------------------------------------------------------------------------
template <typename T>
size_t CTextT<T>::indexOfAny(const T* cList, size_t n, size_t from, bool bCase, size_t* idxList) const
{
...
template <typename T>
template<typename K, typename C, typename V, typename X>
size_t CTextT<T>::indexOfAny(const C& container, size_t n, size_t from, bool bCase, size_t* idxList) const
{
    if(isEmpty())
        return std::string::npos;
    ...
    while(idx < length())
    {
        if(startsWithAny(container, n, idx, bCase, &pos))
        {
            if(idxList)
                *idxList = pos;
            return idx;
...
{
    return indexOfAny(container, container.size(), from, bCase, idxList);
}
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::endsWith(const T& c, size_t from, bool bCase) const
{
    if(isEmpty() || from >= length())
        return false;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
unsigned long CTextT<T>::toBinaryNumber(bool& bOk)
{
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::fromDouble(double d, int precision, bool fixed)
{
    std::basic_stringstream<T> ss;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::readFile(const T* filePath)
{
    return ReadFile(filePath, *this);
}
...
{
    size_t idx = indexOfAny(cList, bCase);
...
{
    return replaceAny(what, what.size(), c, bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
CTextT<T>& CTextT<T>::replaceAny(const C& list, size_t n, const T c, bool bCase)
{
    std::vector< std::pair <size_t, size_t> > v;
    while(findPositions(list, n, v, bCase))
        replaceAt(v, c);
    return *this;
}
...
{
    return replaceAny(container, container.size(), with, bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
CTextT<T>& CTextT<T>::replaceAny(const C& container, size_t n, const T* with, bool bCase)
{
    std::vector< std::pair <size_t, size_t> > v;
    while(findPositions(container, n, v, bCase))
        replaceAt(v, with);
    return *this;
}
...
{
    return replaceAny(what, what.size(), with, bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
CTextT<T>& CTextT<T>::replaceAny(const C& what, size_t n, const C& with, bool bCase)
{
    std::vector< std::pair <size_t, size_t> > v;
    std::vector<size_t> indexes;
    if(findPositions(what, n, v, bCase, &indexes))
...
template <typename T>
template<typename K, typename C, typename V, typename X>
size_t CTextT<T>::findPositions(const C& list, size_t n, RangeVector& pos, bool bCase, std::vector<size_t>* indexes ) const
{
    if(isEmpty())
        return 0;
    ...
    while(idx < length())
    {
        if(startsWithAny(list, n, idx, bCase, &idxList))
        {
            size_t len = Strlen(list[idxList]);
...
template <typename T>
template<typename C, typename Val, typename X>
CTextT<T>& CTextT<T>::reverseAny(const C& container, bool bCase)
{
    return reverseAny(container, container.size(), bCase);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
CTextT<T>& CTextT<T>::reverseAny(const C& list, size_t n,  bool bCase)
{
    std::vector< std::pair <size_t, size_t> > v;
    while(findPositions(list, n, v, bCase))
        reverseAt(v);
...
{
    return removeAny(list, list.size());
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
template<typename K, typename C, typename V, typename X>
CTextT<T>& CTextT<T>::removeAny(const C& list, size_t n, bool bCase)
{
    std::vector< std::pair <size_t, size_t> > v;
    while(findPositions(list, n, v, bCase))
        removeAt(v);
